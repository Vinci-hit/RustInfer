...
RustInfer是一个用Rust语言实现的高性能大语言模型(LLM)推理引擎，手写CUDA算子，**单请求decode吞吐量超越vLLM**。

## 📊 性能对比：RustInfer vs vLLM

> **测试环境**: H20, Qwen3-4B, Batch Size=1, BF16, CUDA Graph enabled, vLLM compile disabled

| | RustInfer | vLLM |
|--|-----------|------|
| **Decode 吞吐量** | **259 tok/s** | 255 tok/s |
| **Decode TPOT** | **3.86ms** | 3.91ms |
| **Prefill TTFT** | 50ms | 47ms |

**v0.6.0 优化路径（192 → 259 tok/s）**:
- 融合 QKV / Gate-Up GEMM（每层 5 次矩阵乘 → 2 次）
- 多 block 并行 argmax（202µs → 5µs）
- 融合 residual-add + RMSNorm（跨层融合，每层 2 处）
...
| Llama-3.2-3B | 3B | GPU | 🔄 未测试 |
| Llama-3.1-8B | 8B | GPU (8GB+) | 🔄 未测试 |

### Qwen3 系列

| 模型 | 参数量 | 推荐设备 | 测试状态 |
|------|--------|----------|----------|
| Qwen3-4B-Instruct | 4B | GPU (8GB+) | ✅ 完全支持 |
...
#### v0.5.0 (当前) - Qwen3 模型支持
**发布日期**: 2026-04

**核心改进**:
- ✨ **Qwen3 4B 推理支持**: 完整的 prefill + decode 双阶段推理
  - Per-head QK-norm（reshape 到 `[seq*heads, head_size]` 后做 RMSNorm）
  - 支持显式 head_dim（Qwen3: head_dim=128 ≠ hidden_size/num_heads）
  - Qwen3 chat template 和 EOS token 识别
- 🚀 **新增 head_dim=128 CUDA 内核**
  - Flash Attention CUTE prefill 内核（hdim128）
  - Flash Decoding BF16 decode 内核（hdim128, 256线程）
- 🔧 **推理修复与优化**
  - 参数化 RoPE theta（CPU/CUDA 双端）
  - CPU Scatter 算子实现
  - CUDA Graph 在 CPU 设备上正确跳过
  - 流式输出中文不再出现乱码（UTF-8 增量安全输出）
  - 采样范围限制到 tokenizer 实际词表大小
- 🧹 **代码清理**: 移除 flashinfer 子模块，清理 debug 打印
