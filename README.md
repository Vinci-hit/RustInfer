...
> **测试说明**:
> - 都是**串行推理模式** (Batch Size=1)，因为本项目目前不支持连续批处理，确保公平竞争
> - 本项目的CUDA算子为**手搓实现，未经高度优化**，仍有巨大的性能提升空间
> - **测试环境**: CUDA v12.2, H200
> - 在更高版本CUDA下性能表现会进一步提升
> - **Rust中除了第一条的TTFT是正常的，其它都有问题！请忽略。**

### RustInfer vs vLLM Prefill性能对比

| RustInfer | vLLM |
|-----------|------|
| ![RustInfer Prefill性能](test_images/rust_seq_test.png) | ![vLLM Prefill性能](test_images/vllm_seq_test.png) |

**性能分析**:
- RustInfer采用手写CUDA核心，未对H系列架构进行优化，计算标志固定是sm89
- vLLM采用经过充分优化的算子库，总体吞吐量更稳定
