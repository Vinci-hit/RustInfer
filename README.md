...
| 指标 | 数值 | 说明 |
|------|------|------|
| 模型加载时间 | ~5秒 | 包含权重映射和初始化 |
| Prefill阶段 | 770 tok/s | 首token时间 |
| 解码吞吐量 | 156 tok/s | 单请求生成速度 |
| 显存占用 | ~5GB | 模型 + KV缓存 |

### 性能优化技术

1. **KV缓存**: 缓存注意力计算中的Key和Value矩阵，避免重复计算
2. **零拷贝加载**: 使用内存映射 (mmap) 直接访问模型权重，无需全部加载到RAM
3. **CUDA优化**: 关键算子使用Cute模板库进行优化
4. **并行计算**: CPU算子使用Rayon进行数据并行
5. **混合精度**: GPU使用BF16加速，CPU使用F32保证精度

**注**: 以上数据仅供参考，实际性能取决于硬件、模型大小和使用场景。
