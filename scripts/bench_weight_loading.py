"""Compare Qwen3.5 weight loading against saved baseline binaries.
...
def main():
    ...
    for repeat in range(-args.warmup_rounds, args.repeats):
        ...
        for variant in variants if repeat % 2 == 0 else reversed(variants):
            ...
            # GPU utilization can still reflect the preceding worker's last
            # request after shutdown. Wait for that sampling window to expire.
            for _ in range(30):
                info = gpu_info(args.gpu)
                if (info["memory_mib"] <= args.max_background_memory_mib
                        and info["utilization"] <= args.max_background_utilization):
                    break
                time.sleep(1)
