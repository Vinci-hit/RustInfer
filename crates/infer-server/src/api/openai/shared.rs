...
pub fn validate_speculative_request(
    draft_tokens: usize,
    ...
    if draft_tokens > 0 {
        if has_image {
            return Err(AppError::bad_request(
                "Speculative decoding currently supports text input only",
            ));
        }
        if temperature.unwrap_or(1.0) > 0.0 && top_k != Some(1) && top_p.unwrap_or(1.0) > 0.0 {
            return Err(AppError::bad_request(
                "Speculative decoding currently requires greedy sampling (temperature=0, top_k=1, or top_p=0)",
            ));
        }
    }
    Ok(())
...
mod tests {
    ...
    fn mtp_request_limits_do_not_change_ordinary_sampling() {
        use super::validate_speculative_request;
        assert!(validate_speculative_request(0, None, None, None, true).is_ok());
        assert!(validate_speculative_request(3, Some(0.0), None, None, false).is_ok());
        assert!(validate_speculative_request(3, None, None, Some(1), false).is_ok());
        assert!(validate_speculative_request(3, None, Some(0.0), None, false).is_ok());
        assert!(validate_speculative_request(3, None, None, None, false).is_err());
        assert!(validate_speculative_request(3, Some(0.0), None, None, true).is_err());
    }
    use super::*;
