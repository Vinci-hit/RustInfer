...
fn mock_control_plane() -> (
    ...
    tokio::sync::mpsc::UnboundedSender<
        crate::infrastructure::transport::control_plane::ControlEvent,
    >,
...
async fn step_output_final_prefill_decodes_with_existing_blocks() -> Result<()> {
    ...
    assert!(
        ...
        "no scheduler control msg expected"
    );
...
async fn step_error_via_control_plane_fails_inflight() -> Result<()> {
    ...
    let result = engine
        .on_control_event(
            crate::infrastructure::transport::control_plane::ControlEvent::StepError {
    ...
    let output = StepOutput {
        ...
        assigned_indices: vec![
            AssignedIndices {
                ...
                base: 0,
                len: 3,
                token_ids: vec![40, 41, 42],
            },
            AssignedIndices {
                sequence_id: 200,
                base: 3,
                len: 5,
                token_ids: vec![50, 51, 52, 53, 54],
            },
        ],
    };
    engine
...
async fn phase8_full_admission_cycle_through_engine() -> Result<()> {
    ...
    for sid in [101u64, 102, 103] {
        ...
        let out = StepOutput {
            ...
            assigned_indices: vec![AssignedIndices {
                ...
                len: 8,
                token_ids: (0..8).map(|i| sid as i32 + i).collect(),
            }],
        };
    ...
    engine
        .handle_step_output_llm(codec.encode(&finish_out)?)
        .await?;
    assert_eq!(
    ...
    let out104 = StepOutput {
        ...
        assigned_indices: vec![AssignedIndices {
            ...
            base: 100,
            len: 16,
            token_ids: (0..16).map(|i| 700 + i).collect(),
        }],
    };
    engine
        .handle_step_output_llm(codec.encode(&out104)?)
        .await?;
...
async fn alloc_failed_round_0_evicts_lru() -> Result<()> {
    ...
    engine
        .on_control_event(
            crate::infrastructure::transport::control_plane::ControlEvent::AllocFailed {
                worker: default_worker.clone(),
                req: AllocFailed {
                    worker_id: "worker-test".to_string(),
                    shortfall: 4,
