...
use crate::cache::traits::{BlockHash, CacheStrategy, PhysicalBlockId, PrefixMatch};
...
impl RadixTreeCache {
    pub fn new(block_size: usize) -> Self {
        ...
        Self {
            inner: Mutex::new(RadixTreeInner {
                block_size,
                nodes: vec![RadixNode {
                    parent: None,
...
impl CacheStrategy for RadixTreeCache {
    fn match_prefix(&self, tokens: &[i32]) -> PrefixMatch {
        let mut inner = self.inner.lock().expect("radix tree cache lock poisoned");
