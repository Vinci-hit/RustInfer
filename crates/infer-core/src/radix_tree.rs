...
/// scheduler and worker sequence identifiers.
pub type SeqId = u64;
...
impl RadixTree {
    ...
    /// Fork a known chain tip without looking up or copying its token prefix.
    /// The child pins the shared nodes before its parent can be released.
    pub fn fork_sequence(&mut self, parent: SeqId, child: SeqId) -> bool {
        if self.seqs.contains_key(&child) {
            return false;
        }
        let Some(tip) = self.seqs.get(&parent).copied() else {
            return false;
        };
        let mut node = tip.leaf;
        while node != self.root {
            self.add_owner(node, child);
            node = self.nodes[node].parent.unwrap();
        }
        self.seqs.insert(child, tip);
        true
    }

    /// Materialize the kernel's flat block table; ownership stays in the tree.
    pub fn sequence_indices(&self, seq: SeqId, out: &mut Vec<GlobalIndex>) -> bool {
        out.clear();
        let Some(tip) = self.seqs.get(&seq) else {
            return false;
        };
        let mut node = tip.leaf;
        let mut end = tip.pos;
        while node != self.root {
            out.extend(self.nodes[node].global_indices[..end].iter().rev().copied());
            node = self.nodes[node].parent.unwrap();
            end = self.nodes[node].global_indices.len();
        }
        out.reverse();
        true
    }
...
mod beam_tests {
    use super::*;
    #[test]
    fn fork_pins_shared_prefix_until_last_child_finishes() {
        let mut tree = RadixTree::new();
        for i in 0..64 {
            tree.append_token(1, i, i as u32);
        }
        assert!(tree.fork_sequence(1, 2));
        assert!(tree.fork_sequence(1, 3));
        assert!(!tree.fork_sequence(1, 3));
        tree.mark_finished_chain(1);
        tree.append_token(2, 100, 64);
        tree.append_token(3, 101, 65);
        let mut indices = vec![];
        assert!(tree.sequence_indices(3, &mut indices));
        assert_eq!(indices, (0..64).chain([65]).collect::<Vec<_>>());
        tree.mark_finished_chain(2);
        assert_eq!(tree.evict(usize::MAX), vec![64]);
        tree.mark_finished_chain(3);
        let mut freed = tree.evict(usize::MAX);
        freed.sort_unstable();
        assert_eq!(freed, (0..64).chain([65]).collect::<Vec<_>>());
        assert_eq!(tree.token_count(), 0);
    }

    #[test]
    fn fork_of_partial_edge_preserves_each_tip_when_it_splits() {
        let mut tree = RadixTree::new();
        for i in 0..8 {
            tree.append_token(1, i, i as u32);
        }
        tree.lookup_prefix(&[0, 1, 2], 2);
        assert!(tree.fork_sequence(2, 3));
        tree.append_token(3, 99, 20);
        for (id, expected) in [
            (1, (0..8).collect()),
            (2, vec![0, 1, 2]),
            (3, vec![0, 1, 2, 20]),
            ...
            let mut got = vec![];
            assert!(tree.sequence_indices(id, &mut got));
