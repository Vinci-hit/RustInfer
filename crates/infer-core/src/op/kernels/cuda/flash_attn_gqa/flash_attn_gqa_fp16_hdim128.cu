#include <cuda_fp16.h>
// flash_attn_gqa_fp16_hdim128.cu
// Flash Attention GQA Prefill kernel for head_dim=128 (FP16)
// Adapted from flash_attn_gqa_fp16.cu (head_dim=64)
//
...
__global__ void flash_attn_gqa_kernel_fp16_hdim128(
    ...
    __half* __restrict__ o_ptr, OStride dO, OSmemLayout sO_layout, TiledCopyO gmem_tiled_copy_O, TiledMma mma)
{
    ...
    extern __shared__ __half shared_memory_128[];
    using SharedStorageType = SharedStorage128<__half, QSmemLayout, KVSmemLayout, KVSmemLayout>;
...
#pragma unroll
        ...
        Tensor rP = convert_type_128<__half>(rS);
...
#pragma unroll
    ...
    Tensor rO = convert_type_128<__half>(acc_o);
    // Reuse sQ shared memory for output (sO has same shape [128, 128])
    Tensor sO = make_tensor(sQ.data(), sO_layout);

    using SmemCopyAtomO = Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, __half>;
    ...
    Tensor tOrO_reg = make_tensor<__half>(shape(tOgO));
    ...
    cudaStream_t stream)
{
    ...
    using AtomGMEM = Copy_Atom<SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>, __half>;
    ...
    using AtomGMEM_Out = Copy_Atom<UniversalCopy<cute::uint128_t>, __half>;
    ...
    using S2RAtom = Copy_Atom<SM75_U32x4_LDSM_N, __half>;
    using S2RAtom_trans = Copy_Atom<SM75_U16x8_LDSM_T, __half>;

    // Grid and block
    dim3 dimGrid(size(ceil_div(seq_len, Int<kBlockM>{})), size(q_heads));
    dim3 block(size(mma));

    int smem_size = int(sizeof(SharedStorage128<__half, decltype(sQ), decltype(sKV), decltype(sKV)>));

    // Request extended shared memory (needed for 128 head_dim: ~64KB > default 48KB)
    auto kernel_ptr = flash_attn_gqa_kernel_fp16_hdim128<
