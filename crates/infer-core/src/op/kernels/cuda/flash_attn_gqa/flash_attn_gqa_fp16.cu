...
__global__ void flash_attn_gqa_kernel_fp16_test(
    ...
    __half* __restrict__ o_ptr,OStride dO, OSmemLayout sO_layout, TiledCopyO gmem_tiled_copy_O, TiledMma mma)
{
    ...
    using SharedStorage = SharedStorage<__half, QSmemLayout, KVSmemLayout,KVSmemLayout>;
...
#pragma unroll
         ...
         Tensor rP = convert_type<__half>(rS);
...
#pragma unroll
    ...
    Tensor rO = convert_type<__half>(acc_o);
    ...
    Tensor tOrO = make_tensor<__half>(shape(tOgO));
    ...
    cudaStream_t stream)
{
    ...
    int smem_size = int(sizeof(SharedStorage<__half, decltype(sQ), decltype(sKV), decltype(sKV)>));
    
    // 8. 启动 Kernel
    flash_attn_gqa_kernel_fp16_test<<<dimGrid, block, smem_size, stream>>>(
        q_shape, kv_shape,
