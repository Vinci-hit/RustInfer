...
__global__ void llama3_1B_decode_kernel_optimized(
    const __half* __restrict__ Q,  // [num_q_heads, D]
    const __half* __restrict__ K,  // [Seq_Len, num_kv_heads, D]
    const __half* __restrict__ V,  // [Seq_Len, num_kv_heads, D]
    __half* __restrict__ O,        // [num_q_heads, D]
    int32_t* seq_len_ptr,
    int num_kv_heads,
    int group_size,
    float sm_scale
    ...
    __half* s_q = smem;
    __half* s_k = s_q + HEAD_DIM_1B;
    __half* s_v = s_k + (2 * BN * HEAD_DIM_1B);
    ...
    auto fetch_kv = [&](int iter, int stage) {
        ...
        if (token_idx < seq_len) {
            const __half* k_ptr = K + (token_idx * num_kv_heads + kv_head_idx) * HEAD_DIM_1B + lane * 8;
            const __half* v_ptr = V + (token_idx * num_kv_heads + kv_head_idx) * HEAD_DIM_1B + lane * 8;
            __half* sk_dst = &s_k[(stage * BN + gid) * HEAD_DIM_1B + lane * 8];
            __half* sv_dst = &s_v[(stage * BN + gid) * HEAD_DIM_1B + lane * 8];
    ...
    for (int i = 0; i < seq_len; i += BN) {
        ...
        if (current_token_idx < seq_len) {
            __half* sk_local = &s_k[(cur_stage * BN + gid) * HEAD_DIM_1B];
            __half* sv_local = &s_v[(cur_stage * BN + gid) * HEAD_DIM_1B];
    ...
    if (tid < 64) {
        ...
        if (final_sum > 0) {
            O[q_head_idx * HEAD_DIM_1B + tid] = __float2half(final_acc_dim / final_sum);
        } else {
            O[q_head_idx * HEAD_DIM_1B + tid] = __float2half(0.0f);
        }
    }
}

extern "C" void flash_decoding_cu_fp16(
    const __half* q_ptr,
    const __half* k_ptr,
    const __half* v_ptr,
    __half* o_ptr,
    ...
    cudaStream_t stream)
{
    ...
    // 约 13KB，远小于硬件限制
    size_t smem_size = (HEAD_DIM_1B * sizeof(__half)) + 
                       (2 * BN * HEAD_DIM_1B * 2 * sizeof(__half)) +
