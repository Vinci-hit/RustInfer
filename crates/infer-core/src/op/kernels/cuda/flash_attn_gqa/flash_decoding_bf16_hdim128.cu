...
__global__ void flash_decode_bf16_hdim128_kernel(
    ...
    __nv_bfloat16* __restrict__ O,        // [num_q_heads, 128]
    int32_t* seq_len_ptr,
    int num_kv_heads,
    int group_size,
    float sm_scale
    ...
    if (tid < HD128) {
        ...
        if (final_sum > 0) {
            O[q_head_idx * HD128 + tid] = __float2bfloat16(final_acc_dim / final_sum);
        } else {
            O[q_head_idx * HD128 + tid] = __float2bfloat16(0.0f);
        }
    }
}
    ...
    cudaStream_t stream)
{
    ...
    flash_decode_bf16_hdim128_kernel<<<num_q_heads, 256, smem_size, stream>>>(
