//! Llama3 model implementation — full forward with attention.
...
pub struct Llama3Model<T: Dtype, D: OpBackend> {
    ...
    /// Precomputed sin cache [max_seq_len, head_dim/2] for RoPE
    pub sin_cache: Tensor<T, D>,
    /// Precomputed cos cache [max_seq_len, head_dim/2] for RoPE
...
impl<T: Dtype, D: OpBackend> LlmModel<T, D> for Llama3Model<T, D> {
    fn forward(
        ...
        for (layer_idx, layer) in self.layers.iter().enumerate() {
            ...
            // Scatter K, V into KV cache for this layer
            D::scatter_kv(
                &k, &v,
                &mut ctx.k_caches[layer_idx],
                &mut ctx.v_caches[layer_idx],
                &positions, kv_dim,
            )?;

            // Attention: Q @ K^T (from cache) → softmax → @ V
...
/// Create a device tensor from host i32 positions.
/// Allocates on the target device and copies data from host.
pub fn alloc_i32<D: OpBackend>(positions: &[i32], dev: &D) -> OpResult<Tensor<i32, D>> {
    let n = positions.len();
    let shape = Shape::from_slice(&[n]);
    let mut tensor = D::alloc_tensor::<i32>(shape, dev)?;
    // Copy host data into the allocated tensor
    let size_bytes = n * 4;
    unsafe {
...
/// Build seq_starts [batch+1] prefix sum from seq_lens.
