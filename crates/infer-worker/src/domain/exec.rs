...
use crate::domain::ports::collective::CommAxis;
use crate::domain::ports::{MemoryPort, OpResult};

#[derive(Clone, Copy, PartialEq, Eq, Hash, Debug)]
pub struct DeviceId(pub i32);

pub trait Device: MemoryPort {
    type Scope: ExecScope<Device = Self>;

    fn device_id(&self) -> DeviceId {
        DeviceId(<Self as crate::domain::ports::Device>::device_id(self))
    }

    fn enter_device(&self) -> DeviceId {
        <Self as Device>::device_id(self)
    }
...
impl<'a, D: Device> ActiveGuard<'a, D> {
...
impl<D: Device> Drop for ActiveGuard<'_, D> {
    fn drop(&mut self) {
...
#[derive(Debug)]
pub struct Workspace<D: Device> {
    _ptr: Option<NonNull<u8>>,
    _size: usize,
    _d: PhantomData<D>,
}

unsafe impl<D: Device> Send for Workspace<D> {}
unsafe impl<D: Device> Sync for Workspace<D> {}

impl<D: Device> Workspace<D> {
