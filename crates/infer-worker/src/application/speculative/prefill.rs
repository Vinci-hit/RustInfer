...
pub struct ShiftedFeatureAlignment<T: Dtype, D: LlmBackend> {
    pending: Option<(i32, Tensor<T, D>)>,
    scratch: Option<AlignmentScratch<T, D>>,
    next_ids: Vec<i32>,
    pair_positions: Vec<i32>,
}
impl<T: Dtype, D: LlmBackend> Default for ShiftedFeatureAlignment<T, D> {
    fn default() -> Self {
        Self {
            pending: None,
            scratch: None,
...
pub struct PreparedFeatureChunk<'a, T: Dtype, D: LlmBackend> {
    state: &'a mut ShiftedFeatureAlignment<T, D>,
    next: (i32, Tensor<T, D>),
...
pub type PreparedMtpChunk<'a, T, D> = PreparedFeatureChunk<'a, T, D>;
impl<T: Dtype, D: LlmBackend> PreparedFeatureChunk<'_, T, D> {
    pub fn commit(self) {
        self.state.pending = Some((self.next.0, self.next.1.clone()));
    }
}

impl<T: Dtype, D: LlmBackend> Drop for PreparedFeatureChunk<'_, T, D> {
    fn drop(&mut self) {
...
impl<T: Dtype, D: LlmBackend> ShiftedFeatureAlignment<T, D> {
    ...
    pub fn prepare_on(
        ...
        hidden: &Tensor<T, D>,
        scope: &D::Scope,
    ) -> OpResult<PreparedFeatureChunk<'_, T, D>> {
    ...
    /// Hidden rows must be head conditioning for target outputs for these tokens.
    ...
    fn prepare_impl(
                ...
                .any(|p| p[0].checked_add(1) != Some(p[1]))
        {
            return Err(OpError::Shape(
                "feature alignment requires nonempty consecutive text positions and [tokens, dim] hidden"
                    .into(),
            ));
        }
        if let Some((p, h)) = &self.pending {
            if p.checked_add(1) != Some(positions[0]) || h.shape().as_slice() != [1, shape[1]] {
                return Err(OpError::Shape(
                    "feature alignment chunk does not follow pending hidden".into(),
                ));
            }
        } else if positions[0] != 0 {
            return Err(OpError::Shape(
                "feature alignment must start at position zero".into(),
            ));
        }
...
/// Compatibility name for existing MTP callers.
pub type MtpPrefill<T, D> = ShiftedFeatureAlignment<T, D>;
...
mod tests {
    ...
    fn every_chunk_partition_preserves_shift_and_hidden_positions() {
        ...
        let all =
            Tensor::from_host_slice(&(0..15).map(|x| x as f32).collect::<Vec<_>>(), [5, 3], &Cpu)
                .unwrap();
        for cuts in 0..16 {
            let mut state = ShiftedFeatureAlignment::default();
    ...
    fn startup_alignment_reuses_buffers_and_aborted_chunk_keeps_carry() {
        let mut state = ShiftedFeatureAlignment::with_capacity(4, 2, &Cpu).unwrap();
