...
unsafe extern "C" {
    fn embedding_kernel_cu_bf16x8(
        output: *mut half::bf16, indices: *const i32, table: *const half::bf16,
        token_len: i32, dim: i32, vocab_size: i32, stream: cudaStream_t,
    );
    fn embedding_kernel_cu_fp16x8(
        output: *mut half::f16, indices: *const i32, table: *const half::f16,
        token_len: i32, dim: i32, vocab_size: i32, stream: cudaStream_t,
    );
    fn embedding_kernel_cu_fp32x4(
        output: *mut f32, indices: *const i32, table: *const f32,
        token_len: i32, dim: i32, vocab_size: i32, stream: cudaStream_t,
    );
}
    ...
    unsafe {
        match T::DATA_TYPE {
            DataType::F32 => embedding_kernel_cu_fp32x4(
                output.data_ptr_mut() as _, indices.data_ptr(), table.data_ptr() as _,
                seq_len, dim, vocab, stream,
            ),
