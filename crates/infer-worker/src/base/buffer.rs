...
impl Buffer {
    ...
    /// Asynchronously copy data from another Buffer using CUDA stream.
    /// This is a non-blocking operation - use cudaStreamSynchronize to wait for completion.
    ///
    /// # Arguments
    /// * `src` - Source buffer to copy from
    /// * `stream` - Optional CUDA stream for async copy. If None, uses default stream.
    ///              For CPU-to-CPU copies, stream parameter is ignored.
    #[cfg(feature = "cuda")]
    pub fn async_copy_from(&mut self, src: &Buffer, stream: Option<crate::cuda::ffi::cudaStream_t>) -> Result<()> {
        if self.len_bytes() != src.len_bytes() {
            return Err(Error::InvalidArgument(format!(
                "Buffer size mismatch: dst has {} bytes, src has {} bytes",
                self.len_bytes(),
        ...
        match (src.device(), self.device()) {
            (DeviceType::Cpu, DeviceType::Cpu) => {
                // CPU to CPU: no stream needed, fallback to synchronous copy
                unsafe {
                    std::ptr::copy_nonoverlapping(
                        src.as_ptr(),
                        self.as_mut_ptr(),
