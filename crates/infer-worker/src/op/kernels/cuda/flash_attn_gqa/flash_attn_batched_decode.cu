...
// -----------------------------------------------------------------------------
// Naive paged decode attention (correctness-first skeleton).
// One CTA per (request, q_head); thread 0 computes the full head serially.
// This is intentionally simple and graph-friendly; optimized split-K paged decode
// can replace it without changing the Rust-facing ABI.
// -----------------------------------------------------------------------------
...
__global__ void paged_decode_naive_kernel(
    ...
    float softmax_scale)
{
    ...
    for (int t = 0; t < kv_len; ++t) {
        ...
        const uint32_t physical_block = block_tables[b * max_blocks_per_seq + block_idx];
        const Elem* k_row = k_pool + ((static_cast<size_t>(physical_block) * block_size + block_off) * num_kv_heads + kvh) * head_dim;
        float score = 0.0f;
    ...
    for (int d = 0; d < head_dim; ++d) {
        ...
        for (int t = 0; t < kv_len; ++t) {
            ...
            const uint32_t physical_block = block_tables[b * max_blocks_per_seq + block_idx];
            const Elem* k_row = k_pool + ((static_cast<size_t>(physical_block) * block_size + block_off) * num_kv_heads + kvh) * head_dim;
            const Elem* v_row = v_pool + ((static_cast<size_t>(physical_block) * block_size + block_off) * num_kv_heads + kvh) * head_dim;
            float score = 0.0f;
        ...
        const float out = denom == 0.0f ? 0.0f : acc / denom;
        o_row[d] = PagedET<Elem>::from_f(out);
    }
}

template <typename Elem>
static inline void launch_paged_decode_naive(
    const Elem* q,
    int64_t qsb,
    int64_t qsh,
    const Elem* k_pool,
    ...
    cudaStream_t stream)
{
    ...
    dim3 block(1);
    paged_decode_naive_kernel<Elem><<<grid, block, 0, stream>>>(
...
extern "C" void launch_flash_attn_paged_decode_bf16(
    ...
    cudaStream_t stream)
{
    launch_paged_decode_naive<__nv_bfloat16>(
        q, qsb, qsh, k_pool, v_pool, o, osb, osh,
        block_tables, max_blocks_per_seq, block_size, kv_lens,
        batch, num_q_heads, num_kv_heads, head_dim, softmax_scale, stream);
}
...
extern "C" void launch_flash_attn_paged_decode_fp16(
    ...
    cudaStream_t stream)
{
    launch_paged_decode_naive<__half>(
        q, qsb, qsh, k_pool, v_pool, o, osb, osh,
        block_tables, max_blocks_per_seq, block_size, kv_lens,
        batch, num_q_heads, num_kv_heads, head_dim, softmax_scale, stream);
}
...
// -----------------------------------------------------------------------------
// Naive paged ragged/prefill attention (correctness-first skeleton).
// One CTA per (packed query token, q_head); thread 0 scans the causal prefix.
// -----------------------------------------------------------------------------

template <typename Elem>
__global__ void paged_ragged_naive_kernel(
    const Elem* __restrict__ q,
    int64_t qss,
    int64_t qsh,
    const Elem* __restrict__ k_pool,
...
template <typename Elem>
static inline void launch_paged_ragged_naive(
    const Elem* q,
    int64_t qss,
    int64_t qsh,
    const Elem* k_pool,
    ...
    cudaStream_t stream)
{
    if (batch <= 0 || total_q_tokens <= 0) return;
    dim3 grid(total_q_tokens, num_q_heads);
    dim3 block(1);
    paged_ragged_naive_kernel<Elem><<<grid, block, 0, stream>>>(
...
extern "C" void launch_flash_attn_paged_ragged_bf16(
    ...
    cudaStream_t stream)
{
    launch_paged_ragged_naive<__nv_bfloat16>(
        q, qss, qsh, k_pool, v_pool, o, oss, osh,
        block_tables, max_blocks_per_seq, block_size, kv_lens, cu_q_lens,
...
extern "C" void launch_flash_attn_paged_ragged_fp16(
    ...
    cudaStream_t stream)
{
    launch_paged_ragged_naive<__half>(
        q, qss, qsh, k_pool, v_pool, o, oss, osh,
        block_tables, max_blocks_per_seq, block_size, kv_lens, cu_q_lens,
        batch, total_q_tokens, num_q_heads, num_kv_heads, head_dim,
        softmax_scale, is_causal, stream);
