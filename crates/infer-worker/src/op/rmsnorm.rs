...
impl RMSNorm {
    /// 创建一个新的 RMSNorm 算子
    pub fn new(dim: usize, dtype: DataType, device: DeviceType, epsilon: f32) -> Result<Self> {
        let weight = Tensor::new(&[dim], dtype, device)?;
        Ok(Self { weight, dim, epsilon })
    }
    pub fn from(weight: Tensor, epsilon: f32) -> Self {
        let dim = weight.shape()[0];
        Self { weight, dim, epsilon }
    }
}
...
impl Op for RMSNorm {
    ...
    fn forward(&self, ctx: &mut OpContext) -> Result<()> {
        ...
        // --- 4. 分派到具体的内核实现 ---
        match input.device() {
            DeviceType::Cpu => kernels::cpu::rmsnorm(input, weight, output, self.epsilon),
            #[cfg(feature = "cuda")]
            DeviceType::Cuda(_) => kernels::cuda::rmsnorm(input, weight, output, ctx.cuda_config, self.epsilon),
        }
    }
}
...
mod tests {
    ...
    fn setup(
        ...
        let op = RMSNorm::new(dim, DataType::F32, device, 1e-5)?;
    ...
    fn test_rmsnorm_bf16_cpu_batch() -> Result<()> {
        ...
        for batch in [1, 2, 4, 8] {
            ...
            let mut rmsnorm_op = RMSNorm::new(dim, dtype, device, 1e-5)?;
    ...
    fn test_rmsnorm_bf16_cpu_vs_cuda() -> Result<()> {
        ...
        for batch in [1, 4, 8] {
            ...
            // CPU computation
            let mut rmsnorm_op_cpu = RMSNorm::new(dim, dtype, DeviceType::Cpu, 1e-5)?;
            ...
            // GPU computation
            let mut rmsnorm_op_gpu = RMSNorm::new(dim, dtype, DeviceType::Cuda(0), 1e-5)?;
