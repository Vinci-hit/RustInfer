...
use infer_protocol::worker_to_scheduler_data::{StepOutput, DiffusionBatchOutput};
...
impl DataPump {
    pub fn new(
        ...
        let recv_socket = ctx.socket(zmq::PULL).map_err(|e| format!("PULL socket: {}", e))?;
        recv_socket.set_linger(1000).map_err(|e| format!("set linger: {}", e))?;
        recv_socket.connect(recv_endpoint).map_err(|e| format!("connect recv {}: {}", recv_endpoint, e))?;

        let send_socket = ctx.socket(zmq::PUSH).map_err(|e| format!("PUSH socket: {}", e))?;
        send_socket.set_linger(1000).map_err(|e| format!("set linger: {}", e))?;
        send_socket.connect(send_endpoint).map_err(|e| format!("connect send {}: {}", send_endpoint, e))?;

        Ok(Self { recv_socket, send_socket })
    }

    /// Blocking receive next batch command.
    pub fn recv_batch(&self) -> Result<BatchCommand, String> {
        let bytes = self.recv_socket.recv_bytes(0).map_err(|e| format!("recv: {}", e))?;
    ...
    pub fn try_recv_batch(&self, timeout_ms: i64) -> Result<Option<BatchCommand>, String> {
        if self.recv_socket.poll(zmq::POLLIN, timeout_ms).map_err(|e| format!("poll: {}", e))? == 0 {
            return Ok(None);
        }
    ...
    pub fn send_step_output(&self, output: &StepOutput) -> Result<(), String> {
        ...
        self.send_socket.send(&bytes, 0).map_err(|e| format!("send: {}", e))
    }
