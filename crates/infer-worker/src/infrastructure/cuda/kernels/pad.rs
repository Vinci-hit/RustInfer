...
unsafe extern "C" {
    fn broadcast_row_bf16_forward(dst: *mut bf16, row: *const bf16, num_rows: i32, d: i32, stream: cudaStream_t);
    fn broadcast_row_f32_forward (dst: *mut f32,  row: *const f32,  num_rows: i32, d: i32, stream: cudaStream_t);
    fn fill_repeat_last_row_bf16_forward(dst: *mut bf16, n_src: i32, target_len: i32, d: i32, stream: cudaStream_t);
    fn fill_repeat_last_row_f32_forward (dst: *mut f32,  n_src: i32, target_len: i32, d: i32, stream: cudaStream_t);
}

unsafe fn d2d(dst: *mut std::ffi::c_void, src: *const std::ffi::c_void, n: usize, stream: cudaStream_t) -> OpResult<()> {
    if n == 0 { return Ok(()); }
    let code = unsafe {
        cudaMemcpyAsync(dst, src, n, cudaMemcpyKind::cudaMemcpyDeviceToDevice, stream)
    };
...
pub fn pad_with_token_into<T: Dtype>(
    ...
    if ss.len() != 2 || ds.len() != 2 || ss[1] != ds[1] {
        return Err(OpError::Shape(format!(
            "pad_with_token: src={:?} dst={:?}", ss, ds,
        )));
    }
    let (n, d) = (ss[0], ss[1]);
    let target = ds[0];
    if target < n {
        return Err(OpError::Shape(format!("pad_with_token: target {} < n {}", target, n)));
    }
    if pad_token.numel() != d {
        return Err(OpError::Shape(format!(
            "pad_with_token: pad_token has {} elems, expected {}", pad_token.numel(), d,
        )));
    }
    ...
    unsafe {
        d2d(dst.data_ptr_mut() as _, src.data_ptr() as _, src_bytes, stream)?;
        if target > n {
            ...
            match T::DATA_TYPE {
                DataType::BF16 => broadcast_row_bf16_forward(
                    ...
                    pad_rows as i32, d as i32, stream,
                ),
                ...
                other => return Err(OpError::Kernel(format!(
                    "pad_with_token: unsupported dtype {:?}", other,
                ))),
            }
        }
    }
...
pub fn pad_last_row_into<T: Dtype>(
    ...
    if ss.len() != 2 || ds.len() != 2 || ss[1] != ds[1] {
        return Err(OpError::Shape(format!(
            "pad_last_row: src={:?} dst={:?}", ss, ds,
        )));
    }
    let (n, d) = (ss[0], ss[1]);
    let target = ds[0];
    if n == 0 {
        return Err(OpError::Shape("pad_last_row: src must have at least one row".into()));
    }
    if target < n {
        return Err(OpError::Shape(format!("pad_last_row: target {} < n {}", target, n)));
    }
    let stream = src.device().config.stream;
    let bytes_per_row = d * T::SIZE_BYTES;
    let src_bytes = n * bytes_per_row;
        ...
        if target > n {
            match T::DATA_TYPE {
                DataType::BF16 => fill_repeat_last_row_bf16_forward(
                    ...
                    n as i32, target as i32, d as i32, stream,
                ),
                ...
                other => return Err(OpError::Kernel(format!(
                    "pad_last_row: unsupported dtype {:?}", other,
                ))),
            }
        }
    }
...
pub fn overwrite_pad_tokens_inplace<T: Dtype>(
    ...
    if keep_prefix > target {
        return Err(OpError::Shape(format!(
            "overwrite_pad: keep_prefix {} > target {}", keep_prefix, target,
        )));
    }
    if pad_token.numel() != d {
        return Err(OpError::Shape(format!(
            "overwrite_pad: pad_token {} elems, expected {}", pad_token.numel(), d,
        )));
    }
    if keep_prefix == target { return Ok(()); }
    ...
    unsafe {
        ...
        match T::DATA_TYPE {
            ...
            other => return Err(OpError::Kernel(format!(
                "overwrite_pad: unsupported dtype {:?}", other,
            ))),
        }
    }
    Ok(())
...
mod tests {
    ...
    fn pad_with_token_f32_basic() {
        ...
        for i in 0..n * d { assert_eq!(got[i], src_host[i]); }
        for r in n..target {
            for c in 0..d {
                assert_eq!(got[r * d + c], pad_host[c],
                    "row {} col {}: got {}, expected {}", r, c, got[r * d + c], pad_host[c]);
            }
        }
    }
    ...
    fn overwrite_pad_tokens_f32_basic() {
        ...
        let mut dst: Tensor<f32, Cuda> = Tensor::from_host_slice(&init, [target, d], &cuda).unwrap();
        let pad: Tensor<f32, Cuda> = Tensor::from_host_slice(&pad_host, [d], &cuda).unwrap();
        let keep = 2usize;
        overwrite_pad_tokens_inplace(&mut dst, &pad, keep).unwrap();
        let got = dst.to_host_vec().unwrap();
        // [0..keep) untouched.
        for i in 0..keep * d { assert_eq!(got[i], init[i]); }
        // [keep..) = pad.
    ...
    fn pad_with_token_bf16_basic() {
        ...
        let n = 1usize;
        let target = 3usize;
        let src_host: Vec<bf16> = vec![1.0, 2.0, 3.0, 4.0].iter().map(|&v| bf16::from_f32(v)).collect();
        let pad_host: Vec<bf16> = vec![-1.0, -2.0, -3.0, -4.0].iter().map(|&v| bf16::from_f32(v)).collect();
        ...
        for c in 0..d { assert_eq!(got[c], src_host[c].to_f32()); }
        for r in n..target {
            for c in 0..d { assert_eq!(got[r * d + c], pad_host[c].to_f32()); }
        }
    }
}
