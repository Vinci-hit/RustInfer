...
unsafe extern "C" {
    fn layernorm_f32_forward(output: *mut f32, input: *const f32, rows: i32, cols: i32, eps: f32, stream: cudaStream_t);
    fn layernorm_bf16_forward(output: *mut half::bf16, input: *const half::bf16, rows: i32, cols: i32, eps: f32, stream: cudaStream_t);
    fn layernorm_f16_forward(output: *mut half::f16, input: *const half::f16, rows: i32, cols: i32, eps: f32, stream: cudaStream_t);
}
...
pub fn layernorm<T: Dtype>(
    ...
    unsafe {
        ...
        match T::DATA_TYPE {
            DataType::F32 => layernorm_f32_forward(output.data_ptr_mut() as _, input.data_ptr() as _, rows, cols, eps, stream),
            DataType::BF16 => layernorm_bf16_forward(output.data_ptr_mut() as _, input.data_ptr() as _, rows, cols, eps, stream),
            DataType::F16 => layernorm_f16_forward(output.data_ptr_mut() as _, input.data_ptr() as _, rows, cols, eps, stream),
            _ => return Err(OpError::Kernel(format!("layernorm: {:?}", T::DATA_TYPE))),
        }

        // Step 2: output *= weight (broadcast multiply)
        match T::DATA_TYPE {
            DataType::F32 => broadcast_mul_f32_forward(output.data_ptr_mut() as _, output.data_ptr() as _, weight.data_ptr() as _, rows, cols, stream),
            DataType::BF16 => broadcast_mul_bf16_forward(output.data_ptr_mut() as _, output.data_ptr() as _, weight.data_ptr() as _, rows, cols, stream),
            DataType::F16 => broadcast_mul_f16_forward(output.data_ptr_mut() as _, output.data_ptr() as _, weight.data_ptr() as _, rows, cols, stream),
            _ => {}
        }

        // Step 3: output += bias (broadcast add)
        match T::DATA_TYPE {
            DataType::F32 => broadcast_add_inplace_f32_forward(output.data_ptr_mut() as _, bias.data_ptr() as _, rows, cols, stream),
            DataType::BF16 => broadcast_add_inplace_bf16_forward(output.data_ptr_mut() as _, bias.data_ptr() as _, rows, cols, stream),
            DataType::F16 => broadcast_add_inplace_f16_forward(output.data_ptr_mut() as _, bias.data_ptr() as _, rows, cols, stream),
            _ => {}
        }
    }
    Ok(())
