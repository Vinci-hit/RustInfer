...
pub fn ewise_mul<T: Dtype>(a: &Tensor<T, Cuda>, b: &Tensor<T, Cuda>, dst: &mut Tensor<T, Cuda>) -> OpResult<()> {
    let n = a.numel() as i32;
    let stream = a.device().config.stream;
    unsafe {
        match T::DATA_TYPE {
            DataType::F32 => ewise_mul_f32_forward(dst.data_ptr_mut() as _, a.data_ptr() as _, b.data_ptr() as _, n, stream),
            DataType::BF16 => ewise_mul_bf16_forward(dst.data_ptr_mut() as _, a.data_ptr() as _, b.data_ptr() as _, n, stream),
            DataType::F16 => ewise_mul_f16_forward(dst.data_ptr_mut() as _, a.data_ptr() as _, b.data_ptr() as _, n, stream),
