...
unsafe extern "C" {
    fn launch_flash_attn_paged_decode_bf16(
        ...
        softmax_scale: f32, stream: cudaStream_t,
    );
    ...
    fn flash_attn_batched_decode_workspace_bytes(batch: i32, num_q_heads: i32, head_dim: i32) -> i64;
...
pub fn flash_decode_workspace_capacity_f32(batch: usize, num_q_heads: usize, head_dim: usize) -> usize {
    let bytes = unsafe {
...
pub fn attention_paged<T: Dtype>(
    ...
    match plan.kind {
        BatchKind::DecodeOnly => {
            ...
            {
                let need = unsafe {
                    flash_attn_batched_decode_workspace_bytes(batch, head_num as i32, head_dim as i32)
                } as usize;
                let have = workspace.numel() * std::mem::size_of::<f32>();
                debug_assert!(have >= need,
                    "attention_paged workspace too small: have {} bytes, need {}", have, need);
            }

            unsafe {
                match T::DATA_TYPE {
                    DataType::BF16 => launch_flash_attn_paged_decode_bf16(
                        q.data_ptr() as _, q_stride_seq, q_stride_head,
                        k_pool.data_ptr() as _, v_pool.data_ptr() as _,
                        output.data_ptr_mut() as _, o_stride_seq, o_stride_head,
                        ...
                        batch, head_num as i32, kv_head_num as i32, head_dim as i32,
                        scale, stream,
                    ),
                    ...
                    _ => return Err(OpError::Kernel(format!("attention_paged DecodeOnly: dtype {:?}", T::DATA_TYPE))),
                }
            }
        }
        BatchKind::Ragged => {
            unsafe {
                match T::DATA_TYPE {
                    DataType::BF16 => launch_flash_attn_paged_ragged_cute_bf16(
                        ...
                        plan.block2req.data_ptr(), plan.block2tile.data_ptr(),
                        plan.total_q_tiles,
                        batch, plan.num_tokens as i32,
                        head_num as i32, kv_head_num as i32, head_dim as i32,
                        scale, 1, stream,
                    ),
                    ...
                    _ => return Err(OpError::Kernel(format!("attention_paged Ragged: dtype {:?}", T::DATA_TYPE))),
                }
            }
        }
    }
